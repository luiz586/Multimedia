...
// distortion & PSNR
//================================================================================================
uint64 xCmp::calcSAD(xCmp* ReferenceCmp)
{
...
double xCmp::calcPSNR(xCmp* ReferenceCmp)
{
  ...
  int64 NumPoints = m_Margin ? m_SizeX*m_SizeY : m_BufferSize;
  int64 MaxValue =(1<<m_BitDepth) - 1;
...
uint64 xCmp::predict_4x4(xCmp* Src)
{
  _declspec(align(32)) int16 Left [9];
  _declspec(align(32)) int16 Above[9];
  _declspec(align(32)) int16 Pred [4*4];

  uint64 TotalTicks = 0;

  int16* SrcPtr = Src->m_PelOrg;
  int16* DstPtr = m_PelOrg;
  int32 SrcStride = Src->m_PelStride;
  int32 DstStride = m_PelStride;
  
  SrcPtr += (4*SrcStride);
  DstPtr += (4*DstStride);
  
  for(int32 y=4; y<m_SizeY-4; y+=4)
  {
    for(int32 x=4; x<m_SizeX-4; x+=4)
    {
      for(int32 l=-1; l<8; l++) { Above[l+1] = *(SrcPtr-SrcStride+l+x); }
      for(int32 l=-1; l<8; l++) { Left [l+1] = *(SrcPtr+(SrcStride*l)-1+x); }

      uint64 Ticks = __rdtsc();
      //xIntraPred::PredictAngular_NxN_STD(Left+1, Above+1, Pred, 4, 4, 15);
      xIntraPred::PredictAngular_4x4_SSE(Left+1, Above+1, Pred, 4, 4, 15);
      TotalTicks += (__rdtsc() - Ticks);    

      for(int32 l=0; l<4; l++) memcpy(DstPtr+x+l*DstStride, &Pred[4*l], 4*sizeof(int16));
    }
    SrcPtr += (4*SrcStride);
    DstPtr += (4*DstStride);
  }
...
uint64 xCmp::predict_8x8(xCmp* Src)
{
  _declspec(align(32)) int16 Left [17];
  _declspec(align(32)) int16 Above[17];
  _declspec(align(32)) int16 Pred [8*8];
  ...
  for(int32 y=8; y<m_SizeY-8; y+=8)
  {
    for(int32 x=8; x<m_SizeX-8; x+=8)
    {
      for(int32 l=-1; l<16; l++) { Above[l+1] = *(SrcPtr-SrcStride+l+x); }
      for(int32 l=-1; l<16; l++) { Left [l+1] = *(SrcPtr+(SrcStride*l)-1+x); }

      uint64 Ticks = __rdtsc();
      xIntraPred::PredictAngular_NxN_STD(Left+1, Above+1, Pred, 8, 8,  14);
      //xIntraPred::PredictAngular_8x8_SSE(Left+1, Above+1, Pred, 8, 8,  14);
      TotalTicks += (__rdtsc() - Ticks);    

      for(int32 l=0; l<8; l++) memcpy(DstPtr+x+l*DstStride, &Pred[8*l], 8*sizeof(int16));
    }
    SrcPtr += (8*SrcStride);
    DstPtr += (8*DstStride);
  }
...
uint64 xCmp::predict_16x16(xCmp* Src)
{
  _declspec(align(32)) int16 Left [33];
  _declspec(align(32)) int16 Above[33];
  _declspec(align(32)) int16 Pred [16*16];
  ...
  for(int32 y=16; y<m_SizeY-16; y+=16)
  {
    for(int32 x=16; x<m_SizeX-16; x+=16)
    {
      for(int32 l=-1; l<32; l++) { Above[l+1] = *(SrcPtr-SrcStride+l+x); }
      for(int32 l=-1; l<32; l++) { Left [l+1] = *(SrcPtr+(SrcStride*l)-1+x); }

      uint64 Ticks = __rdtsc();
      //xIntraPred::PredictAngular_NxN_STD(Left+1, Above+1, Pred, 16, 16,  14);
      xIntraPred::PredictAngular_16x16_SSE(Left+1, Above+1, Pred, 16, 16,  14);

      TotalTicks += (__rdtsc() - Ticks);    

      for(int32 l=0; l<16; l++) memcpy(DstPtr+x+l*DstStride, &Pred[16*l], 16*sizeof(int16));
    }
    SrcPtr += (16*SrcStride);
    DstPtr += (16*DstStride);
  }
...
uint64 xCmp::predict_32x32(xCmp* Src)
{
  _declspec(align(32)) int16 Left [65];
  _declspec(align(32)) int16 Above[65];
  _declspec(align(32)) int16 Pred [32*32];
  ...
  for(int32 y=32; y<m_SizeY-32; y+=32)
  {
    for(int32 x=32; x<m_SizeX-32; x+=32)
    {
      for(int32 l=-1; l<64; l++) { Above[l+1] = *(SrcPtr-SrcStride+l+x); }
      for(int32 l=-1; l<64; l++) { Left [l+1] = *(SrcPtr+(SrcStride*l)-1+x); }

      uint64 Ticks = __rdtsc();
      //xIntraPred::PredictAngular_NxN_STD(Left+1, Above+1, Pred, 32, 32,  14);
      xIntraPred::PredictAngular_NxN_SSE(Left+1, Above+1, Pred, 32, 32,  14);
      TotalTicks += (__rdtsc() - Ticks);    

      for(int32 l=0; l<32; l++) memcpy(DstPtr+x+l*DstStride, &Pred[32*l], 32*sizeof(int16));
    }
    SrcPtr += (32*SrcStride);
    DstPtr += (32*DstStride);
  }
...
uint64 xCmp::predict_64x64(xCmp* Src)
{
  _declspec(align(32)) int16 Left [128];
  _declspec(align(32)) int16 Above[128];
  _declspec(align(32)) int16 Pred [64*64];
  ...
  for(int32 y=64; y<m_SizeY-64; y+=64)
  {
    for(int32 x=64; x<m_SizeX-64; x+=64)
    {
      for(int32 l=-1; l<128; l++) { Above[l+1] = *(SrcPtr-SrcStride+l+x); }
      for(int32 l=-1; l<128; l++) { Left [l+1] = *(SrcPtr+(SrcStride*l)-1+x); }

      uint64 Ticks = __rdtsc();
      //xIntraPred::PredictAngular_NxN_STD(Left+1, Above+1, Pred, 64, 64, 30);
      xIntraPred::PredictAngular_NxN_SSE(Left+1, Above+1, Pred, 64, 64, 30);
      TotalTicks += (__rdtsc() - Ticks);    

      for(int32 l=0; l<64; l++) memcpy(DstPtr+x+l*DstStride, &Pred[64*l], 64*sizeof(int16));
    }
    SrcPtr += (64*SrcStride);
    DstPtr += (64*DstStride);
  }
