...
void xImg::createResizedCompatibile(xImg* RefImg, int newWidth, int newHeight){
  m_ImageType    = RefImg->m_ImageType;
  m_ChromaFormat = RefImg->m_ChromaFormat;
  m_ColorSpace   = RefImg->m_ColorSpace;
  m_BitDepth     = RefImg->m_BitDepth;
  ...
  {
    m_Cmp[CmpIdx] = new xCmp;
	RefImg->m_Cmp[CmpIdx]->m_SizeX = newWidth;
	RefImg->m_Cmp[CmpIdx]->m_SizeY = newHeight;
    m_Cmp[CmpIdx]->createCompatible(RefImg->m_Cmp[CmpIdx]);
  }
}
...
void xImg::unpackRAW(uint8* Src)
{
  ...
  for(int32 k=0; k<3; k++)
  {    
    int16* PelPtr   = m_Cmp[k]->m_PelOrg;
    int32  PelSride = m_Cmp[k]->m_PelStride;
    int32  SizeX    = m_Cmp[k]->m_SizeX;
    int32  SizeY    = m_Cmp[k]->m_SizeY;
    int32  Points   = SizeY*SizeX;

    switch (BitDepthMultiplier)
    {
    case 1:      
      if(m_Cmp[k]->m_Margin) xPixel::UnpackU8ToI16(PelPtr, Src, PelSride, SizeX, SizeX, SizeY);
      else                   xPixel::UnpackU8ToI16Continuous(PelPtr, Src, Points);
      break;
    case 2:
      if(m_Cmp[k]->m_Margin)
      {
      ...
      {
          memcpy(PelPtr, SrcTmp, SizeX*BitDepthMultiplier);
          PelPtr += PelSride;
          SrcTmp += SizeX*BitDepthMultiplier;
      }
      }
      else
      {
        memcpy(PelPtr, Src, Points*BitDepthMultiplier);
      }
      break;
    }  
...
void xImg::packRAW(uint8* Dst)
{
  ...
  {    
    ...
    {
    case 1:
      if(m_Cmp[k]->m_Margin) xPixel::PackI16ToU8(Dst, PelPtr, SizeX, PelSride, SizeX, SizeY);
      else                   xPixel::PackI16ToU8Continuous(Dst, PelPtr, Points);
      break;
    case 2:
      if(m_Cmp[k]->m_Margin)
      {
        ...
        for(int32 PosY=0; PosY < SizeY; PosY++)
        {
          memcpy(DstTmp, PelPtr, SizeX*BitDepthMultiplier);
          PelPtr += PelSride;
          DstTmp += SizeX*BitDepthMultiplier;
      }
      }
      else
        {
        memcpy(Dst, PelPtr, Points*BitDepthMultiplier);
        }
      break;
    }
